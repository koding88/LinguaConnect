...
const followUser = async (follower, following) => {
    try {
        ...
        const { error } = followUserValidation({
            follower: follower,
            following: following,
        });
        if (error) {
...
const updateAvatar = async (userId, avatar) => {
    try {
        ...
        logger.error(
            `Error updating avatar for user ${userId}: ${error.message}`
        );
        throw error;
    }
};
