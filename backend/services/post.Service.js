...
const getPostByUserId = async (userId) => {
    try {
        // Validate userId
        const { error } = getPostValidation({ id: userId });
        if (error) {
            throw errorHandler(400, `Validation error: ${error.message}`);
        }
        ...
        // Log success
        logger.info(`Successfully retrieved ${posts.length} posts for user ${userId}`);
        
        return posts;
    } catch (error) {
        logger.error(`Error getting posts by user ID: ${error.message}`);
        throw error;
    }
};
...
const createPost = async (userId, postData) => {
    try {
        ...
        if (!contentSafety.isSafe) {
            throw errorHandler(
                400,
                `You violated the following categories: ${contentSafety.violatedCategories.join(
                    ", "
                )}`
            );
        }
...
const updatePost = async (postId, userId, updateData) => {
    try {
        ...
        const { error: idError } = postIdValidation({
            postId: postId,
            userId: userId,
        });
        if (idError) {
        ...
        const { error: validationError } = updatePostValidation({
            content: content,
            urls: urls,
            images: images,
        });
        ...
        if (urls && Array.isArray(urls) && urls.length > 0) {
            ...
            if (invalidUrls.length > 0) {
                throw errorHandler(
                    400,
                    `The following URLs do not exist in this post: ${invalidUrls.join(
                        ", "
                    )}`
                );
            }
...
const filterPostByComments = async (userId) => {
    try {
        ...
        const postIds = [
            ...new Set(userComments.map((comment) => comment.post)),
        ];
